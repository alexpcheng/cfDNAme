...
Unlike demuxbyname, this should allow unlimited output files, but may be slower.

Usage:
demuxbyname2.sh in=<file> in2=<file2> out=<outfile> out2=<outfile2> names=<string,string,string...>

Alternately:
demuxbyname2.sh in=<file> out=<outfile> delimiter=whitespace prefixmode=f
This will demultiplex by the substring after the last whitespace.

demuxbyname2.sh in=<file> out=<outfile> length=8 prefixmode=t
...
substring=f     Names can be substrings of read headers.  Substring mode
                is slow if the list of names is large; use prefix, suffix,
                or column mode if possible.
