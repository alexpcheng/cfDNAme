...
//		v.homopolymerCount=hmp; //derived
	...
	public ArrayList<VCFLine> split(boolean splitAlleles, boolean splitSubs){
		assert(splitAlleles || splitSubs);
		if(!compound()){splitAlleles=false;}
//		if(type!=Var.SUB || alt.length<2){splitSubs=false;}
		...
		if(type!=Var.SUB || alt.length<2){splitSubs=false;}
		if(!splitSubs || compound()){return null;}//Can't split subs with commas
...
//			line.rehash();
	...
	private int trimToCanonical(){
		if(compound() || alt.length<2 || ref.length<2 /*|| ref.length==alt.length*/){return 0;}
...
//	}
	...
	public int type_old(){
		if(alt!=null && Tools.indexOf(alt, ',')>=0) {return Var.COMPOUND;}
