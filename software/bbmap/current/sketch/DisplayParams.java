...
//		printColors=true;
	...
	public JsonObject toJson(Comparison c){
		...
		if(printWKID){j.add("WKID", 100*c.wkid());}
		if(printKID){j.add("KID", 100*c.kid());}
	...
	ByteBuilder queryHeader(Sketch sk){
		...
		if(printQueryFileName && sk.fname()!=null){sb.append("\tFile: "+sk.fname());}
...
//		if(printUContam){sb.append(String.format(Locale.ROOT, "\t%.2f%%", 100*c.uContamFraction()));}
		...
		if(printEValue){bb.tab().append(String.format("%5.2e", c.eValue()));}
