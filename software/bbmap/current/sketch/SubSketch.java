...
//		outstream.println(inSketches.get(0));
		...
		if(ffout!=null){
			...
			for(Sketch sk : inSketches){
				if(sk.length()>targetSketchSize){
					sk.resize(targetSketchSize);
				}
				if(!defaultParams.trackCounts()){sk.counts=null;}
				sumOut+=sk.length();
				sk.toBytes(bb);
				bsw.print(bb);
				bb.clear();
			...
			t.stop();
			outstream.println("Wrote "+inSketches.size()+" sketches of total size "+sumOut+" in \t"+t);
		}
