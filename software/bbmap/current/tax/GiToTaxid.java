...
//		System.err.println("j: "+number);
	...
	static int parseTaxidNumber(String s, char delimiter){
	...
	public static int getID(String s, char delimiter){
		int x=parseGiNumber(s, delimiter);
		if(x>=0){return array[x];}
		return parseTaxidNumber(s, delimiter);
	}
...
//		Arrays.fill(x, -1);
	...
	private static long addToList(String fname, IntList list){
		...
		while(line!=null){
			if(Tools.isDigit(line[line.length-1])){//Invalid lines will end with tab or na
				count++;
...
//				assert(false) : tab2+", "+tab3+", '"+new String(line)+"'";
				int tid=Tools.parseInt(line, tab2+1, tab3);
				int gi=Tools.parseInt(line, tab3+1, line.length);
				int old=list.get(gi);
