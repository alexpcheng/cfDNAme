	...
	public RenameGiToTaxid(String[] args){
		...
		for(int i=0; i<args.length; i++){
			...
			if(a.equals("prefix")){
				...
				maxInvalidHeaders=Tools.parseKMG(b);
...
//			System.gc();
	...
	void process(Timer t){
		...
		for(FileFormat ffin : ffin1){
			...
			if(useServer){
				processInner_server(bf, bsw, bswInvalid, bswBadHeaders, counts);
			}else{
				IntList list=(useServer ? getIds(bf) : null);
				processInner(bf, bsw, bswInvalid, bswBadHeaders, counts, list);
			}
		}
	...
	private void updateHeadersFromServer(ArrayList<byte[]> lines, HashArray1D counts, ByteStreamWriter bswBadHeaders){
		ByteBuilder bb=new ByteBuilder();
		for(byte[] line : lines){
...
//		System.err.println("Sending '"+bb+"'");
	...
	private void processInner_server(ByteFile bf, ByteStreamWriter bsw, ByteStreamWriter bswInvalid, ByteStreamWriter bswBadHeaders,  HashArray1D counts){
		...
		while(line!=null){
			...
			if(line.length>0){
				...
				if(storedBytes>=maxStoredBytes){
					updateHeadersFromServer(lines, counts, bswBadHeaders);
